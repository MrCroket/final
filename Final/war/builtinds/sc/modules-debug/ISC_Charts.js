...
,isc.A._initExtraAxes=function isc_FacetChart__initExtraAxes(){
    ...
    for(var j=0;j<len;++j){
        ...
        if(metricSettings.matchGradations!=null){
            var k=metrics.findIndex(metricSettings.matchGradations),
                match=(
                    k!=-1?
                    settings[k]:
...
isc.evalBoundary;isc.B.push(isc.A.drawScatterData=function isc_FacetChart_drawScatterData(rotateXLabels){
    var xMetric=this.getXAxisMetric(),
...
,isc.A.getNearestDrawnValue=function isc_FacetChart_getNearestDrawnValue(x,y,metric){
    ...
    var res=this._getNearestDrawnValue(x,y,context,true,false,false);
    if(res!=null){
...
,isc.A.getNearestDrawnValues=function isc_FacetChart_getNearestDrawnValues(x,y){
    ...
    var res;
    if(this.chartType=="Scatter"){
        res=this._getScatterNearestDrawnValues(x,y,false,false);
    }else{
        var dataLabelFacet=this.getDataLabelFacet(),
            legendFacet=this.getLegendFacet(),
        ...
        for(var i=0;i<len;++i){
            ...
            res[i]=context&&this._getNearestDrawnValue(x,y,context,true,false,false);
        }
    }
...
,isc.A._getScatterNearestDrawnValues=function isc_FacetChart__getScatterNearestDrawnValues(x,y,keepText,keepDrawnXAndY){
    ...
    if(nearestDrawnValue==null){
        ...
        for(var i=len;i--;){
            if(i==xIndex){
                var value=nearestDrawnValue.record[xAxisMetric];
                res[i]={
                    x:Math.round(nearestDrawnValue.drawnX),
                    y:(keepDrawnXAndY?Math.round(nearestDrawnValue.drawnY):null),
                    value:value,
                    facetValues:facetValues,
                ...
                var value=nearestDrawnValue.record[yAxisMetric];
                res[i]={
                    x:(keepDrawnXAndY?Math.round(nearestDrawnValue.drawnX):null),
...
,isc.A._getNearestDrawnValue=function isc_FacetChart__getNearestDrawnValue(x,y,context,byDistanceOnly,keepText,keepDrawnXAndY){
    context=context||this;
    if(context.chartType=="Scatter"){
        var nearestDrawnValues=this._getScatterNearestDrawnValues(x,y,keepText,keepDrawnXAndY);
...
isc.evalBoundary;isc.B.push(isc.A._getDrawnValue=function isc_FacetChart__getDrawnValue(context,i,j){
    var data=this.getOrderedData(),
        legendFacet=this.getLegendFacet(),
        ...
        y>rect.top&&y<(rect.top+rect.height))
    {
        ...
        if(context!=null){
            if(this._hoverLabel==null){
                ...
                {
                    this._dataPoint=this.drawDataPoint([-100,-100],0,6,null,0,null,null,true);
                }
            ...
            var nearestValue=this._getNearestDrawnValue(x,y,context,false,true,true);
                                ...
                                if(this.knobs[i].dataValue==nearestValue.value){
                                    ...
                                    if(equals){
                                        ...
                                        if(this._dataPoint==null){
                                            ...
                                            var color=this._dataPoint.fillColor;
                                            this._dataPoint.prevFillColor=color;
                                            color=this._getHighlightedColor(color,this.brightenPercent);
                                            this._dataPoint.setFillColor(color);
                                            var color=this._dataPoint.lineColor;
                                            this._dataPoint.prevLineColor=color;
                                            color=this._getHighlightedColor(color,this.brightenPercent);
                                            this._dataPoint.setLineColor(color);
                                        }
                                        break;
                                    }
                                }
...
,isc.A._cleanDataPoint=function isc_FacetChart__cleanDataPoint(){
    var context=this._getShowValueOnHoverContext();
    if(context.showDataPoints==true){
        this._dataPoint.setFillColor(this._dataPoint.prevFillColor);
        this._dataPoint.setLineColor(this._dataPoint.prevLineColor);
        ...
        init:function(chart,context,minAxisWidth){
            ...
            for(var j=context.length;j--;){
                ...
                if(chart.chartType=="Scatter"){
                    ...
                    minVal=Math.min(0,chart.getMinValue(metric));
    ...
    return{
        init:function(chart,context,minAxisWidth){
            ...
            var axis=this._gradationsAxis=isc.FacetChart._createLinearGradationsAxis(
                    false,vertical,!(vertical||chart._isZoomChart()));
...
,isc.A._createLabelCollapseModeSampleAxis=function isc_c_FacetChart__createLabelCollapseModeSampleAxis(vertical){
    return{
        ...
        measure:function(){
            var chart=this._chart,
                showInlineLabels=chart.showInlineLabels,
                reserve=!(vertical||chart._isZoomChart()),
