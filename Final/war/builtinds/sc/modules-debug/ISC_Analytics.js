...
/*
  ...
  Version v9.0p_2013-10-13 (2013-10-13)
...
if(isc.Log && isc.Log.logDebug)isc.Log.logDebug(isc._pTM.message,'loadTime');
...
else isc._preLog=[isc._pTM]}isc.definingFramework=true;if(window.isc&&isc.version!="v9.0p_2013-10-13"){
    isc._optionalModuleCompare=function(){
        ...
        if(isc.version.toLowerCase().contains("pro")||isc.version.toLowerCase().contains("lgpl")){
            ...
            var moduleVersion="v9.0p_2013-10-13";
        ...
        if(incompatibleVersions){
            isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
                +"SmartClient version '"+isc.version+"' and additional modules from 'v9.0p_2013-10-13'. Mixing resources from different "
...
,isc.A.setFieldProperty=function isc_CubeGrid_setFieldProperty(field,property,value,noData){
    ...
    this.markValuesPresent(facetValues,facets,true);
    var props=this.getFieldProperties(facetValues,facets);
    if(!props){
        props={};
        if(noData)props.noData=true;
    }else{
        if(!noData&&props.noData)delete props.noData;
        else if(noData)props.noData=true;
    }
    props[property]=value;
}
,isc.A.markValuesPresent=function isc_CubeGrid_markValuesPresent(cellRecord,facetIds,leaveBlanks){
    ...
    for(var i=0;i<facetIds.length;i++){
        ...
        if(valuesPresent)delete valuesPresent.noData;
    }
}
...
isc.evalBoundary;isc.B.push(isc.A.makeFieldTreeLevel=function isc_CubeGrid_makeFieldTreeLevel(facetValues,level,parentField,facetValueGroup){
    ...
    for(var i=0;i<facetValues.length;i++){
            ...
            this.canCollapseFacets||this.canMinimizeFacets||this.multiRowHeaders)
        {
            ...
            if(this.hideEmptyFacetValues&&!this.attributesOnly()&&
                ...
                !(fieldProps&&fieldProps._retainWhenEmpty)
                )
            {
            ...
            if(fieldProps&&fieldProps._retainWhenEmpty){
                fieldProps._usedRetainFlag=true;
            }
...
,isc.A.addRecordsToIndex=function isc_CubeGrid_addRecordsToIndex(data,startIndex,endIndex,releaseFillers){
    startIndex=startIndex||0;
    endIndex=endIndex||data.length;
    this.logInfo("indexing from "+startIndex+" through "+endIndex,"cellIndexing");
    var start=isc.timeStamp();
    if(this.cellIndex==null)this.cellIndex=[];
    ...
    for(var i=startIndex;i<endIndex;i++){
        ...
        if(releaseFillers){
            ...
            if(oldCellRecord._isFiller)this._fillerCells.remove(oldCellRecord);
        }
...
isc.evalBoundary;isc.B.push(isc.A.bodyDrawing=function isc_CubeGrid_bodyDrawing(){
...
,isc.A._cgFetchData=function isc_CubeGrid__cgFetchData(fetchNow,delayed,callback,requestProperties){
    ...
    if(callback!=null){
        if(dsRequest.clientContext==null)dsRequest.clientContext={};
        dsRequest.clientContext.fetchCallback=callback;
    }
...
,isc.A._lastFetchDataReply=function isc_CubeGrid__lastFetchDataReply(dsResponse,data,dsRequest){
    ...
    if(this.hideEmptyFacetValues){
        var facetValues=this._loadingFacetValueMap;
        var descendants=this._loadingDescendants;
        var facet=this._loadingFacet;
        this._loadingFacetValueMap=null;
        this._loadingDescendants=null;
        this._loadingFacet=null;
        if(descendants){
            for(var i=0;i<descendants.length;i++){
                facetValues[facet.id]=descendants[i].id;
                var fieldProps=this.getFieldProperties(facetValues);
                if(fieldProps&&fieldProps._retainWhenEmpty){
                    delete fieldProps._retainWhenEmpty;
                }
            }
        }
        if(this.hideEmptyAxis==null){
            this.setRows();
            this.setColumns();
        }else if(this.hideEmptyAxis=="row"){
            this.setRows();
        }else{
            this.setColumns();
        }
    ...
    var callback=dsRequest.clientContext?dsRequest.clientContext.fetchCallback:null;
    if(callback){
...
,isc.A.syncHeaderScrolling=function isc_CubeGrid_syncHeaderScrolling(bodyLeft,bodyTop){
...
isc.evalBoundary;isc.B.push(isc.A.createRowFacetLabels=function isc_CubeGrid_createRowFacetLabels(){
    if(!this.haveRowFacets())return;
    var rowFacetLabels=[],
        rowFacetWidths=this.getRowFacetWidths();
    for(var i=0;i<this.rowFields.length;i++){
...
,isc.A._toggleOpenStateForField=function isc_CubeGrid__toggleOpenStateForField(field,isRow,desiredOpen){
    ...
    if(!nowClosed&&this.hideEmptyFacetValues&&!fieldProps._loaded){
        var facet=field.facet,
            valueTree=facet._valueTree,
            facetValue=this.getFacetValue(facet,field.facetValueId),
            descendants=valueTree?valueTree.getDescendants(facetValue):null;
        this._loadingFacet=facet;
        this._loadingFacetValueMap=facetValues;
        this._loadingDescendants=descendants;
        if(descendants){
            for(var i=0;i<descendants.length;i++){
                facetValues[facet.id]=descendants[i].id;
                var childFieldProps=this.getFieldProperties(facetValues);
                if(childFieldProps&&childFieldProps._usedRetainFlag)continue;
                if(childFieldProps)childFieldProps._retainWhenEmpty=true;
            }
        }
    }
...
isc.evalBoundary;isc.B.push(isc.A.selectAllCells=function isc_CubeGrid_selectAllCells(){
...
isc.evalBoundary;isc.B.push(isc.A.addRowFacet=function isc_CubeGrid_addRowFacet(facetId,index){
...
,isc.A.handleUpdate=function isc_CubeGrid_handleUpdate(data,dsRequest,dsReponse){
    ...
    switch(operationType){
    case"add":
        this.addRecordsToIndex(checkedData,null,null,true);
        break;
    case"remove":
